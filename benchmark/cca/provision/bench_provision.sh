#!/usr/bin/env bash
# bench_provision.sh — `tee-anchor provision --tee-type cca`（組織エンドースメント
# 証明書の発行）のプロセスレベル計測。SGX/TDX 版 (benchmark/{sgx,tdx}/provision) の
# Arm CCA 版。
#
# 計測対象:
...
#
# 内訳 (支配項):
#   CCA Attestation Token(CBOR tag399) のパース → Platform Token(COSE_Sign1) を pin
#   した CPAK(ES384/P-384) で署名検証 → claims から Chip ID(instance-id, UEID) 抽出
#   → 組織 CA 鍵で X.509 エンドースメント証明書を署名・書出。
#   ※ SGX/TDX が Quote 内蔵 PCK 証明書チェーンを辿るのに対し、CCA は pin 済み CPAK
#     公開鍵で COSE 署名を直接検証する点が異なる（ネットワーク往復は無し）。
#     フェアな TEE 横断比較のため SGX/TDX 版と同条件 (warmup/runs) で測る。
#
# 既存ファイルの影響排除:
...
#   ※ Token は計測対象外で「既に存在する前提」。CCA マシン(Realm)から持参した
#     cca-token.cbor をファイル入力する（このベンチでは生成しない）。
#
# 計測ツール:
...
# 使い方:
#   ./bench_provision.sh
#   TOKEN=/path/to/cca-token.cbor ./bench_provision.sh
#   BUNDLE_DIR=/path/to/dir ./bench_provision.sh         # $BUNDLE_DIR/cca-token.cbor を使う
...
log()  { echo -e "\033[1;32m[bench-provision-cca]\033[0m $*"; }
warn() { echo -e "\033[1;33m[bench-provision-cca]\033[0m $*" >&2; }
die()  { echo -e "\033[1;31m[bench-provision-cca]\033[0m $*" >&2; exit 1; }
...
# ---------------------------------------------------------------------------
# 入力 CCA Token の探索（既に存在する前提でファイル入力する）
# ---------------------------------------------------------------------------
TOKEN="${TOKEN:-}"
if [ -z "$TOKEN" ]; then
    for c in \
        "${BUNDLE_DIR:-}/cca-token.cbor" \
        "$HOME/cca_verify_bundle/cca-token.cbor" \
        "$SCRIPT_DIR/cca-token.cbor"; do
        [ -n "$c" ] && [ -f "$c" ] && { TOKEN="$c"; break; }
    done
fi
[ -n "$TOKEN" ] && [ -f "$TOKEN" ] \
    || die "CCA Token が見つかりません。TOKEN=<path> で指定してください（例: CCA マシンから持参した cca-token.cbor）。"
...
if "$TEE_ANCHOR" provision --tee-type cca --token "$TOKEN" \
    ...
    die "provision が失敗しました。Token/CA/バイナリの整合を確認してください: $TOKEN"
fi

log "tee-anchor : $TEE_ANCHOR"
log "token      : $TOKEN ($(stat -c%s "$TOKEN" 2>/dev/null || echo '?') bytes)"
...
if command -v hyperfine >/dev/null 2>&1; then
    ...
    for p in "$TEE_ANCHOR" "$TOKEN" "$CA_KEY" "$CA_CERT" "$OUT_CRT"; do
