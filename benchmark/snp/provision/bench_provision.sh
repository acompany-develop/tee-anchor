#!/usr/bin/env bash
# bench_provision.sh — `tee-anchor provision --tee-type snp`（組織エンドースメント
# 証明書の発行）のプロセスレベル計測。SGX/TDX/CCA 版 (benchmark/{sgx,tdx,cca}/provision)
# の AMD SEV-SNP 版。
#
# 計測対象:
...
# 入力 (bench_snp.sh と同じ流儀)
BUNDLE_DIR="${BUNDLE_DIR:-$HOME/snp_verify_bundle}"
REPORT="${REPORT:-$BUNDLE_DIR/report.bin}"
CERTS_DIR="${CERTS_DIR:-$BUNDLE_DIR/certs}"

log()  { echo -e "\033[1;32m[bench-provision-snp]\033[0m $*"; }
warn() { echo -e "\033[1;33m[bench-provision-snp]\033[0m $*" >&2; }
die()  { echo -e "\033[1;31m[bench-provision-snp]\033[0m $*" >&2; exit 1; }
...
# ---------------------------------------------------------------------------
[ -f "$REPORT" ]    || die "SNP Report がありません: $REPORT (REPORT=<path> で指定してください)"
[ -d "$CERTS_DIR" ] || die "certs ディレクトリがありません: $CERTS_DIR (CERTS_DIR=<dir> で指定してください)"
