#!/usr/bin/env bash
# bench_provision.sh — `tee-anchor provision --tee-type sgx`（組織エンドースメント
# 証明書の発行）のプロセスレベル計測。
#
# 計測対象:
#   tee-anchor provision --tee-type sgx --report <quote> \
#       --ca-key <ca.key> --ca-cert <ca.crt> --out <endorsement.crt>
#
# 内訳 (支配項):
#   SGX Quote のパース → CERT_DATA から PCK 証明書チェーン抽出 → SGX 拡張から
#   Chip ID(PPID) 抽出 → 組織 CA 鍵で X.509 エンドースメント証明書を署名・書出。
#   ネットワーク往復は無し（PCK は Quote 内蔵分のみ使用）。
#
# 既存ファイルの影響排除:
...
# 使い方:
#   ./bench_provision.sh
#   QUOTE=/path/to/quote.dat ./bench_provision.sh
...
log()  { echo -e "\033[1;32m[bench-provision]\033[0m $*"; }
warn() { echo -e "\033[1;33m[bench-provision]\033[0m $*" >&2; }
die()  { echo -e "\033[1;31m[bench-provision]\033[0m $*" >&2; exit 1; }
...
[ -n "$QUOTE" ] && [ -f "$QUOTE" ] \
    || die "SGX Quote が見つかりません。QUOTE=<path> で指定してください（例: Humane-RAFW-MAA の RA で生成した quote.dat）。"
...
$PREP_RM
if "$TEE_ANCHOR" provision --tee-type sgx --report "$QUOTE" \
        --ca-key "$CA_KEY" --ca-cert "$CA_CERT" --out "$OUT_CRT" >/dev/null 2>&1; then
    if "$TEE_ANCHOR" verify --tee-type sgx --report "$QUOTE" \
    ...
    die "provision が失敗しました。Quote/CA/バイナリの整合を確認してください: $QUOTE"
fi
...
if command -v hyperfine >/dev/null 2>&1; then
    ...
    hyperfine -N \
        ...
        --min-runs "$RUNS" \
        --prepare "$PREP_RM" \
        --command-name "provision (sgx)" \
            "$TEE_ANCHOR provision --tee-type sgx --report $QUOTE --ca-key $CA_KEY --ca-cert $CA_CERT --out $OUT_CRT" \
    ...
    bench_one "provision (sgx)" "$TEE_ANCHOR" provision --tee-type sgx --report "$QUOTE" \
